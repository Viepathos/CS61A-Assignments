...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                self.place = place
                place.ant = self
            elif not place.ant.can_contain(self) and not self.can_contain(place.ant):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.notyetdoubled:
            self.damage *= 2
            self.notyetdoubled = False
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place is not None:
            if current_place.bees and not current_place.is_hive and self.lower_bound <= total <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        super().reduce_health(amount)  #先要让它掉血啊！！！
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if not other.is_container and self.ant_contained is None and self.is_container:
            return True
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for i in all_bees:
            if i is not None and self is not None:
                Insect.reduce_health(i, self.damage)
