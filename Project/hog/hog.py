...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    rightmost_digit = player_score % 10
    tens_digit = opponent_score // 10

    if tens_digit // 10 != 0 :
        tens_digit = tens_digit % 10

    if rightmost_digit == tens_digit :
        return 1
    else:
        return 3 * abs(tens_digit - rightmost_digit)
    # END PROBLEM 2
...
def num_factors(n):
    """R:eturn the number of factors of N, including 1 and N itself."""
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if is_prime(n) :
        return 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    base_situation = strategy(0, 0)
    s0 = 0
    while s0 < goal :
        s1 = 0
        while s1 < goal :
            if strategy(s0, s1) != base_situation :
                return False
            s1 += 1
        s0 += 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while k <= 10:
        current_score = average_dice(k, dice)
